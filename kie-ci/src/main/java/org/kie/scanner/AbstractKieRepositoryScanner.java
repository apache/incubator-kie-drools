...
public abstract class AbstractKieRepositoryScanner implements InternalKieScanner {
    ...
    InternalArtifactResolver artifactResolver;
    ...
    protected abstract InternalArtifactResolver getArtifactResolver();
    ...
    public synchronized KieModule loadArtifact(ReleaseId releaseId, InputStream pomXml) {
        InternalArtifactResolver resolver = pomXml != null ?
                                    getResolverFor(pomXml) :
                                    getArtifactResolver();
        return loadArtifact( releaseId, resolver );
    }

    public synchronized KieModule loadArtifact(ReleaseId releaseId, PomModel pomModel) {
        InternalArtifactResolver resolver = pomModel != null ?
                                    getResolverFor(pomModel) :
                                    getArtifactResolver();
        return loadArtifact( releaseId, resolver );
    }

    private KieModule loadArtifact( ReleaseId releaseId, InternalArtifactResolver resolver ) {
    ...
    private KieModule loadPomArtifact(ReleaseId releaseId) {
        InternalArtifactResolver resolver = getResolverFor(releaseId, false);
        if (resolver == null) {
            return null;
        }
    ...
    private InternalKieModule buildArtifact(Artifact artifact, InternalArtifactResolver resolver) {
    ...
    private void addDependencies(InternalKieModule kieModule, InternalArtifactResolver resolver, List<DependencyDescriptor> dependencies) {
    ...
    private class ScanTask extends TimerTask {
        public void run() {
            synchronized (AbstractKieRepositoryScanner.this) {
    ...
    private Map<ReleaseId, DependencyDescriptor> indexArtifacts(InternalArtifactResolver artifactResolver) {
    ...
    protected abstract InternalArtifactResolver getResolverFor(InputStream pomXml);
    protected abstract InternalArtifactResolver getResolverFor(PomModel pom);
    protected abstract InternalArtifactResolver getResolverFor(AFReleaseId releaseId, boolean allowDefaultPom);
    protected abstract InternalArtifactResolver getResolverFor(InternalKieContainer kieContainer, boolean allowDefaultPom);
}
