...
Here, only the computers without processes running on them are penalized.
...
The matches from `ifExists()` and other similar building blocks are not available further down the stream.
