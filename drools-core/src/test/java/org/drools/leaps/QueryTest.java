...
public class QueryTest extends DroolsTestCase {
			...
			public void evaluate(Activation activation,
				...
				try {
					...
					if (dummy == QueryTest.this.context1) {
						QueryTest.this.handlesForRules
								.add(QueryTest.this.handle1Rule2);
					} else if (dummy == QueryTest.this.context2) {
						QueryTest.this.handlesForRules
								.add(QueryTest.this.handle2Rule2);
					}
	...
	public void testAddRuleBeforeFacts() throws Exception {
		...
		assertEquals(2, this.handlesForRules.size());
		assertEquals(2, this.workingMemory.getQueryResults("query-to-execute").size());
