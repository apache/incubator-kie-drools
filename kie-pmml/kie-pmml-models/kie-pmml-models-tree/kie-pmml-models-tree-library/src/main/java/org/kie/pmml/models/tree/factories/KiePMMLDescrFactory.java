...
public class KiePMMLDescrFactory {
    ...
    public static PackageDescr getBaseDescr(DataDictionary dataDictionary, TreeModel model, String packageName) throws KieEnumException {
    ...
    private static void declareRules(PackageDescrBuilder builder, Node node, String parentPath) throws KieEnumException {
        ...
        if (node instanceof LeafNode) {
            ...
            rhs = String.format("modify(" + STATUS_HOLDER + ") {" +
                                        ...
                                        "}", currentRule);
            ruleBuilder.rhs(rhs);
            node.getNodes().forEach(throwingConsumerWrapper(child -> declareRules(builder, child, currentRule)));
        }
    }

    private static void declarePredicate(final CEDescrBuilder<?, ?> lhsBuilder, final Predicate predicate) throws KieEnumException {
        String constraint;
        if /*(predicate instanceof True) {
            ...
            predicatesByClass
                    .forEach(throwingBiConsumerWrapper((aClass, predicates) -> {
                        if (SimplePredicate.class.equals(aClass)) {
                            ...
                            predicatesByField.forEach(throwingBiConsumerWrapper((fieldName, simplePredicates) -> declarePredicate(andBuilder, fieldName, simplePredicates, booleanOperator)));
                        } else {
                            predicates.forEach(throwingConsumerWrapper(child -> declarePredicate(andBuilder, child)));
                        }
                    }));
            andBuilder.end();
        }
    ...
    private static void declarePredicate(final CEDescrBuilder<?, ?> lhsBuilder, final String fieldName, final List<SimplePredicate> predicates, final CompoundPredicate.BooleanOperator booleanOperator) throws KieEnumException {
        ...
        switch (booleanOperator) {
            ...
            case SURROGATE:
            default:
                predicates.forEach(throwingConsumerWrapper(predicate -> {
                    OPERATOR operator = OPERATOR.byName(predicate.getOperator().value());
                    String constraint = String.format("value %s \"%s\"", operator.getOperator(), predicate.getValue() != null ? predicate.getValue() : "");
    ...
    private static void declareTypes(PackageDescrBuilder builder, DataDictionary dataDictionary) throws
            KieEnumException {
        dataDictionary.getDataFields().forEach(throwingConsumerWrapper(dataField -> declareType(builder, dataField)));
    }

    private static void declareType(PackageDescrBuilder builder, DataField dataField) throws KieEnumException {
