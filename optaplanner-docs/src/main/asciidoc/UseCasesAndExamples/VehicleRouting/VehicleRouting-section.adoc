...
There are also several technologies to render it, such as http://leafletjs.com[Leaflet] and https://developers.google.com/maps/[Google Maps for developers]. The https://www.optaplanner.org/learn/rhpds/optaweb-vehicle-routing.html[OptaWeb Vehicle Routing project] has an example which demonstrates such rendering:
