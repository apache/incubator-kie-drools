...
Because of JBoss Modules' `ClassLoader` magic, you'll likely need to provide the `ClassLoader` of your classes <<solverConfigurationByXML,during the SolverFactory creation>>,
...
Because of the OSGi's `ClassLoader` magic, you'll likely need to provide the `ClassLoader` of your classes <<solverConfigurationByXML,during the SolverFactory creation>>,
so it can find the classpath resources (such as the solver config, score DRL's and domain classes) in your jars.
