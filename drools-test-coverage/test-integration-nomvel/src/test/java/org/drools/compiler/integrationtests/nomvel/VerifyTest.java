...
public class VerifyTest {
    ...
    public void verify_kieContainer_executableModel() throws Exception {
        ...
        assertThat(results.getMessages(Message.Level.WARNING).size()).isEqualTo(1);

        Results resultsWithKieBaseName = kieContainer.verify("defaultKieBase");
        assertThat(resultsWithKieBaseName.getMessages(Message.Level.WARNING).size()).isEqualTo(1);
    }

    @Test
    public void verify_kieContainer_executableModel_withoutDRL() throws Exception {

        TestUtil.KieFile kieFile = new TestUtil.KieFile("src/main/resources/my.txt", "ABC");

        final KieContainer kieContainer = getKieContainer(null, kieFile);
