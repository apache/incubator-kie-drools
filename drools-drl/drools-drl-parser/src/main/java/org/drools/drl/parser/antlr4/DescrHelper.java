...
public class DescrHelper {
    ...
    private static <T extends BaseDescr> T populateCommonProperties(T descr, DRLParserRuleContext ctx) {
        descr.setStartCharacter(ctx.getStartCharacter());
        descr.setEndCharacter(ctx.getEndCharacter());
        descr.setLocation(ctx.getLine(), ctx.getColumn());
        descr.setEndLocation(ctx.getEndLine(), ctx.getEndColumn());
        return descr;
    }
