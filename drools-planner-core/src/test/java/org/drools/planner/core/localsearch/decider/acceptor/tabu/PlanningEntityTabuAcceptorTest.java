...
public class PlanningEntityTabuAcceptorTest {
    ...
    private MoveScope buildMoveScope(LocalSearchStepScope stepScope, TestdataEntity entity) {
        return buildMoveScope(stepScope, entity, 0);
    }

    private MoveScope buildMoveScope(LocalSearchStepScope stepScope, TestdataEntity entity, int score) {
        MoveScope moveScope = new MoveScope(stepScope);
        moveScope.setMove(new GenericChangeMove(entity, mock(FactHandle.class), mock(PlanningVariableDescriptor.class),
                new TestdataValue("mock")));
