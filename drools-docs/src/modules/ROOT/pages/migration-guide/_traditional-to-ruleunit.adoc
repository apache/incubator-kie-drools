...
In addition, you need to take care of `insert` in RHS. `insert` method inserts a fact into "DEFAULT" entry-point, but in Rule Unit use cases, you should specify a `DataSource` where you want to insert the fact. Hence, you need to call `DataStore.add` or `DataStream.append` instead of `insert`.
