    ...
    public FEELFnResult<BigDecimal> invoke(@ParameterName("n") BigDecimal n, @ParameterName("scale") BigDecimal scale) {
        ...
        scaleObj.ifPresentOrElse(scaleInt -> {
            ...
            if (scaleInt < -6111 || scaleInt > 6176) {
                ...
                toReturn.set(FEELFnResult.ofResult(n.setScale(scaleInt, RoundingMode.HALF_UP)));
            }
        }, () -> toReturn.set(FEELFnResult.ofError(new InvalidParametersEvent(Severity.ERROR, "scale", "should be a non-null Number"))));
        return toReturn.get();
    }
}
