...
{PRODUCT} has 3 types of engine dependency that aggregates a required collection of dependencies.

*`drools-ruleunits-engine`* is the standard engine for rule units that enables executable model.

*`drools-engine`* is the standard engine for traditional DRL syntax that enables executable model.

*`drools-engine-classic`* is the old engine that utilizes an MVEL interpreter.  The `drools-engine-classic` and `drools-mvel` are now *deprecated*, so use `drools-ruleunits-engine` or `drools-engine` as possible.
