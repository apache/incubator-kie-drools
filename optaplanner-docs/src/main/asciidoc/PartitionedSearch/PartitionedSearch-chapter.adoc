...
If you're running in a environment that doesn't like arbitrary thread creation,
...
Then add a public setter for each custom property, which is called when a `Solver` is built.
