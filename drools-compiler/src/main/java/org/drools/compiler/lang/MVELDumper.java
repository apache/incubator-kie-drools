...
public class MVELDumper extends ReflectiveVisitor implements ExpressionRewriter {
    ...
    String[] processImplicitConstraints(String expr, AtomicExprDescr atomicExpr, MVELDumperContext context) {
        ...
        while (sharpPos > 0 || nullSafePos > 0) {
            if (nullSafePos < 0 || (sharpPos > 0 && sharpPos < nullSafePos)) {
                ...
                String[] constrSplitByAnd = constrAndExpr[0].split("&&");
                String previous = getPreviousConstraint(constrSplitByAnd);
                constrAndExpr = new String[]{constrAndExpr[0] + previous + nullCheckAndExpr[0], expr};
            }
    ...
    private String getPreviousConstraint(String[] constrSplitByAnd){
        boolean isAdditionalConstraint =  constrSplitByAnd[0] == "";
        String previous = constrSplitByAnd[0];

        if(!isAdditionalConstraint){
            int notEqualsPosition = constrSplitByAnd[0].indexOf("!=");
            String propertiesToBeAppendedTo = constrSplitByAnd[0].substring(0, notEqualsPosition).trim();
            return propertiesToBeAppendedTo + ".";
        }
    ...
    private String[] processNullSafeDereferencing(String expr, AtomicExprDescr atomicExpr, int nullSafePos) {
        ...
        String[] nullCheckAndExpr = null;
        int lastParanthesisPos = field1.lastIndexOf("(");
        int lastDotPos = field1.lastIndexOf(".");
        String preCondition = getPreconditionsToAppend(field1, lastParanthesisPos, lastDotPos);
        nullCheckAndExpr = new String[]{preCondition, expr};
        atomicExpr.setRewrittenExpression(nullCheckAndExpr[1]);
        return nullCheckAndExpr;
    }

    private String getPreconditionsToAppend(String field1, int lastParanthesisPos, int lastDotPos) {
        if (lastParanthesisPos > lastDotPos) {
            return field1.substring(lastParanthesisPos + 1) + " != null && ";
        }
        return field1.substring(lastDotPos + 1) + " != null && ";
