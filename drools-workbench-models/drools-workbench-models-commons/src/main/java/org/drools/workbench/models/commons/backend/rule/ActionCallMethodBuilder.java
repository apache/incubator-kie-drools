...
public class ActionCallMethodBuilder {
    ...
    public ActionCallMethodBuilder(
            RuleModel model,
    ...
    private List<ActionFieldFunction> getActionFieldFunctions() {
        ...
        for (String param : parameters) {
            ...
            actionFieldFunctions.add(
                    getActionFieldFunction(
                            param,
                            getDataType(param, index)));
        }
        return actionFieldFunctions;
    }

    private String getAdjustedParameter(String param, String dataType) {
        String adjustedParam;
        if (boundParams.containsKey(param)) {
            adjustedParam = param;
        } else {
            adjustedParam = adjustParam(dataType, param, isJavaDialect);
        }
    ...
    private ActionFieldFunction getActionFieldFunction(
            String param,
            String dataType) {
        ActionFieldFunction actionFiled = new ActionFieldFunction(null, getAdjustedParameter(param, dataType), dataType);
        actionFiled.setNature(inferFieldNature(param, boundParams));
        actionFiled.setField(methodName);
        return actionFiled;
    }

    private String getDataType(String param, int index) {
        String dataType;
        ...
        if (dataType == null) {
            return inferDataType(param, isJavaDialect);
        } else {
            return dataType;
        }
    }
        ...
        if (variableType != null) {
            ...
            if (methods != null) {
                for (MethodInfo method : methods) {
                    if (method.getName().equals(methodName) && compareParameters(method.getParams())) {
                        return method;
                    }
                }
            }
