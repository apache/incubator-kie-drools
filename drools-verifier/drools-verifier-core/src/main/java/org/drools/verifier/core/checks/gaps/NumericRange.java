...
public class NumericRange extends Range<Double> implements Comparable<Range<Double>> {
    ...
    protected Consumer<ConditionInspector> getConditionParser() {
        return c -> {
            final FieldCondition condition = (FieldCondition) c.getCondition();
            final Operator operator = resolve(condition.getOperator());
            switch (operator) {
                case LESS_OR_EQUAL:
                case LESS_THAN:
                    upperBound = ((Number) condition.getValues().iterator().next()).doubleValue();
                    break;
                case GREATER_THAN:
                case GREATER_OR_EQUAL:
                    lowerBound = ((Number) condition.getValues().iterator().next()).doubleValue();
                    break;
                case EQUALS:
                default:
                    lowerBound = ((Number) condition.getValues().iterator().next()).doubleValue();
