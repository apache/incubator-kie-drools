...
The design patterns in this section list and solve common design challenges.
//Need to explain what design patterns are.
...
.. Normalize the diagram to remove duplicate data.
// what does it mean to normalize a diagram?
.. Create sample instances for each class. For example, in the Employee Rostering starter application, the samples for the `Employee` class are `Ann`, `Bert`, and `Carl`.

. *Determine which relationships (or fields) change during planning and color them orange.* One side of these relationships will become a planning variable later on. For example, in the Employee Rostering starter application, the `Shift` to `Employee` relationship changes during planning, so it is orange. However, other relationships, such as from `Employee` to `Skill`, are immutable during planning
...
[NOTE]
====
Only one side of a bidirectional relationship can be a genuine planning variable.The other side will become an <<bidirectionalVariable,inverse relation shadow variable>> later on. Keep bidirectional relationships orange.
====
...
====
//I suggest removing this for now, the when you do implement `@PlanningVariable` add this note saying why you shouldn't use it: Although a future version will support it for flexibility reasons. it probably has an inherit performance and complexity cost, so it might be better to avoid it anyway.
+
...
. *Annotate a many-to-one relationship with a `@PlanningEntity` annotation.* Usually the _many_ side of the relationship is the planning entity class that contains the planning variable. If the relationship is bidirectional, both sides are a planning entity class but usually the _many_ side has the planning variable and the _one_ side has the shadow variable. For example, in the Employee Rostering starter application, the `ShiftAssignment` class has an `@PlanningEntity` annotation.
...
.. Remove any surplus `@PlanningVariable` annotations so that they become  problem properties. Doing this significantly decreases <<searchSpaceSize,the search space size>> and significantly increases solving efficiency. For example, in the Employee Rostering starter application, the `ShiftAssignment` class should not annotate both the `Shift` and `Employee` relationship with `@PlanningVariable`.
...
** In some cases, multiple planning entity instances have the same set of problem properties. In such cases, it can be useful to create an extra problem property to distinguish them. For example, in the Employee Rostering starter application, the `ShiftAssignment` class has the problem property `Shift` as well as the problem property `indexInShift` which is an `int` class.

. *Choose the model in which the number of planning entities is fixed during planning*. For example, in the Employee Rostering starter application, it is impossible to know in advance how many shifts each employee will have before Optaplanner solves the model and the results can differ for each solution found.
...
// Not clear just what this diagram is showing:

image::DesignPatterns/entityVariableAndValueExamples.png[align="center"]

[NOTE]
====
Vehicle routing is an exception because it uses a <<chainedPlanningVariable,chained planning variable>>.
====
