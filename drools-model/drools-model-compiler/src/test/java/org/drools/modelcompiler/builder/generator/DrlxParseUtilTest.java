...
public class DrlxParseUtilTest {
    ...
    public void removeRootNodeTest() {
        assertEquals(new RemoveRootNodeResult(of(expr("$a")), expr("getAge()")), removeRootNode(expr("$a.getAge()")));
        assertEquals(new RemoveRootNodeResult(of(expr("$c")), expr("convert($length)")), removeRootNode(expr("$c.convert($length)")));
    }
