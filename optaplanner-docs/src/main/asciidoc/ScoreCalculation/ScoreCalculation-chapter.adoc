...
The example above uses the `ofHard()`, `ofMedium()` and `ofSoft()` methods do that.
...
Each constraint weight links with a constraint implementation, for example in <<droolsScoreCalculation,Drools score calculation>>:
