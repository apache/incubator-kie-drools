...
packagedef : PACKAGE name=qualifiedIdentifier SEMICOLON? ;

unitdef : UNIT name=qualifiedIdentifier SEMICOLON? ;

importdef : IMPORT (FUNCTION|STATIC)? qualifiedIdentifier (DOT STAR)? SEMICOLON? ;

globaldef : GLOBAL type ID SEMICOLON? ;

ruledef : RULE name=stringId (EXTENDS stringId)? annotation* attributes? WHEN lhs THEN rhs END ;
...
lhsPattern : xpathPrimary (OVER patternFilter)? |
             ( QUESTION? qualifiedIdentifier LEFT_PAREN positionalConstraints? constraints? RIGHT_PAREN (OVER patternFilter)? (FROM patternSource)? ) ;
*/

lhsPattern : QUESTION? objectType=qualifiedIdentifier LEFT_PAREN positionalConstraints? constraints? RIGHT_PAREN ;
...
conditionalOrExpression : left=conditionalAndExpression (DOUBLE_PIPE right=conditionalAndExpression)* ;
conditionalAndExpression : left=inclusiveOrExpression (DOUBLE_AMPER right=inclusiveOrExpression)* ;
inclusiveOrExpression : left=exclusiveOrExpression (PIPE right=exclusiveOrExpression)* ;
exclusiveOrExpression : left=andExpression (XOR right=andExpression)* ;
andExpression : left=equalityExpression (AMPER right=equalityExpression)* ;
equalityExpression : left=instanceOfExpression ( ( op=EQUALS | op=NOT_EQUALS ) right=instanceOfExpression )* ;
instanceOfExpression : left=inExpression ( 'instanceof' right=type )? ;
inExpression : left=relationalExpression ( 'not'? 'in' LEFT_PAREN expression (COMMA expression)* RIGHT_PAREN )? ;
relationalExpression : TEXT? ; // TODO

expression : conditionalExpression ( op=assignmentOperator right=expression )? ;
conditionalExpression : left=conditionalOrExpression ternaryExpression? ;
ternaryExpression : QUESTION ts=expression COLON fs=expression ;
...
argument : ( stringId | FLOAT | BOOL | NULL ) ;

annotation : AT name=qualifiedIdentifier arguments? ;

attributes : attribute ( COMMA? attribute )* ;
attribute : ( 'salience' DECIMAL )
          | ( 'enabled' | 'no-loop' | 'auto-focus' | 'lock-on-active' | 'refract' | 'direct' ) BOOLEAN?
          | ( 'agenda-group' | 'activation-group' | 'ruleflow-group' | 'date-effective' | 'date-expires' | 'dialect' ) STRING
          |   'calendars' STRING ( COMMA STRING )*
          |   'timer' ( DECIMAL | TEXT )
          |   'duration' ( DECIMAL | TEXT ) ;

assignmentOperator : EQUALS_ASSIGN
                   |   PLUS_ASSIGN
                   |   MINUS_ASSIGN
                   |   MULT_ASSIGN
                   |   DIV_ASSIGN
