...
public interface Move<Solution_> {
    ...
    Move createUndoMove(ScoreDirector<Solution_> scoreDirector);
...
[source,java,options="nowrap"]
----
    public void doMoveOnGenuineVariables(ScoreDirector<NQueens> scoreDirector) {
...
[NOTE]
====
Change multiple entities or multiple planning variables in a single move to create a coarse-grained move.
====
...
In the example above the undo move of _C0 to C2_ would be the move __C2 to C0__.
...
Notice that if C0 would have already been moved to C2, the undo move would create the move __C2 to C2__, instead of the move __C2 to C0__.
