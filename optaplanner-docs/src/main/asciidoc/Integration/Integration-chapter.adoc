...
Normally OptaPlanner just works on Spring Boot, simply add the `optaplanner-core` dependency.
...
Because they are <<immovablePlanningEntities,immovable>>, OptaPlanner does not change them: it optimizes the planning around the enforcements made by the human.
