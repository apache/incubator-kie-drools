...
[IMPORTANT]
====
The DMN designer in {PRODUCT} {PRODUCT_VERSION} is a Technology Preview feature and is disabled by default in {CENTRAL}. Technology Preview features are not supported with Red Hat production service level agreements (SLAs), might not be functionally complete, and are not recommended for production. These features provide early access to upcoming product features, enabling customers to test functionality and provide feedback during the development process.

To enable the DMN designer preview in {CENTRAL}, in the upper-right corner of the window, click *Settings* -> *Roles*, select a role from the left panel, click *Editors* -> *DMN Designer* -> *Read*, and click *Save* to save the changes.
