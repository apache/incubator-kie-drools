...
public class SingleRangeCheck extends CheckBase {
    ...
    public boolean check() {
        ...
        this.hasIssues = !errors.isEmpty();
        return this.hasIssues;
    }

    private void checkCondition(final int conditionIndex) {
        final Map<OperatorType, Set<ConditionParentType>> fieldsByOperatorType = ruleInspectors.stream()
        ...
        final Set<ConditionParentType> rangeFields = fieldsByOperatorType.get(OperatorType.RANGE);
    ...
    private void checkRanges(final Collection<ConditionParentType> rangeFields,
        ...
        for (Map.Entry<PartitionKey, List<RuleInspector>> partition : partitions.entrySet()) {
            ...
            for (final ConditionParentType field : rangeFields) {
    ...
    private <T extends Comparable> void checkMonodimensionalRange(final Entry<PartitionKey, List<RuleInspector>> partition,
                                                                  ...
                                                                  final ConditionParentType field,
                                                                  final int conditionIndex,
    ...
    private <T extends Comparable> ArrayList<MissingRange> findMissingRanges(final ConditionParentType field,
                                                                             final T max,
        ...
        while (lower.compareTo(max) < 0) {
            ...
            result.add(new MissingRange(field.getName(),
                                        lower,
                                        upper));
    ...
    private OperatorType getFieldOperatorType(final ConditionParentType field,
                                              final int conditionIndex) {
    ...
    private Map<PartitionKey, List<RuleInspector>> getPartitions(final Collection<ConditionParentType> partitionFields,
    ...
    private PartitionKey getPartitionKey(final Collection<ConditionParentType> partitionFields,
    ...
    private PartitionKey makePartitionKey(final Collection<ConditionParentType> partitionFields,
                                          final RuleInspector rule,
                                          final int conditionIndex) {

        final List<Object> list = new ArrayList<>();
        final List<PartitionCondition> conditions = new ArrayList<>();
        for (final ConditionParentType field : partitionFields) {
            final Object value = getValue(rule,
                                          field,
                                          conditionIndex);
            list.add(value);
            conditions.add(new PartitionCondition(field.getName(),
                                                  value));
        }
    ...
    private List<ConditionInspector> getConditions(final RuleInspector rule,
                                                   final ConditionParentType field,
                                                   final int conditionIndex) {
        return rule.getConditionsInspectors() != null ? getList(rule, field, conditionIndex) : null;
    }

    private LeafInspectorList<ConditionInspector> getList(RuleInspector rule, ConditionParentType field, int conditionIndex) {
        return rule.getConditionsInspectors().get(conditionIndex).get(field);
    }
