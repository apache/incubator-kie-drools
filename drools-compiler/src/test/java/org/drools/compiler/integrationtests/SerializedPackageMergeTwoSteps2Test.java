...
public class SerializedPackageMergeTwoSteps2Test {
    ...
    public void testBuildAndSerializePackagesInTwoSteps2() throws IOException, ClassNotFoundException    {        
        ...
        for(String fileName : SerializedPackageMergeTwoSteps1Test.BINPKG) {
            Collection<InternalKnowledgePackage> kpkgs=null;
            byte[] data=null;
            try {
        ...
        Collection<InternalKnowledgePackage> deserializedPackages = (Collection<InternalKnowledgePackage>) in.readObject();
  	...
  	private Collection<InternalKnowledgePackage>  _deserializeFromBytes(byte [] byteCode){
		Collection<InternalKnowledgePackage> ret = null;
		ByteArrayInputStream bis = null;
		ObjectInput in = null;
		try {
			bis = new ByteArrayInputStream(byteCode);
			in = new DroolsObjectInputStream(bis);
			ret =(Collection<InternalKnowledgePackage>)in.readObject();
		} catch (Exception ex) {
			ex.printStackTrace();
		} finally {
			try {
