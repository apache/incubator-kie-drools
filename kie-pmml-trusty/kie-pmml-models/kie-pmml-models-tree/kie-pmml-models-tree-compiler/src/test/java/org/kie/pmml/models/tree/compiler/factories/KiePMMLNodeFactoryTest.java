...
public class KiePMMLNodeFactoryTest {
    ...
    public void getKiePMMLNodeSourcesMap() {
        Map<String, String> retrieved = KiePMMLNodeFactory.getKiePMMLNodeSourcesMap(node1, dataDictionary1,
                                                                                    PACKAGE_NAME);
        assertNotNull(retrieved);
    ...
    public void populateJavaParserDTOAndSourcesMap() {
        ...
        KiePMMLNodeFactory.populateJavaParserDTOAndSourcesMap(toPopulate, sourcesMap, nodeNamesDTO, dataDictionary2, isRoot);
        MethodDeclaration retrieved = CommonCodegenUtils.getMethodDeclaration(toPopulate.nodeTemplate, EVALUATE_PREDICATE).orElseThrow(() -> new RuntimeException("No EVALUATE_PREDICATE in generated class"));
    ...
    public void populateEvaluatePredicate() {
        ...
        KiePMMLNodeFactory.NodeNamesDTO nodeNamesDTO = new KiePMMLNodeFactory.NodeNamesDTO(node);
        ...
        MethodDeclaration retrieved = CommonCodegenUtils.getMethodDeclaration(toPopulate.nodeTemplate, EVALUATE_PREDICATE + nodeNamesDTO.nodeClassName).orElseThrow(() -> new RuntimeException("No EVALUATE_PREDICATE in generated class"));
        commonVerifyEvaluatePredicateMethodDeclaration(retrieved, nodeNamesDTO.nodeClassName, isRoot);

        // root node
        isRoot = true;
        node = new NodeMock(true, dataField.getName().getValue(), dataField.getDataType());
        nodeNamesDTO = new KiePMMLNodeFactory.NodeNamesDTO(node);
        toPopulate = new KiePMMLNodeFactory.JavaParserDTO(nodeNamesDTO, packageName);
        KiePMMLNodeFactory.populateEvaluatePredicate(toPopulate, dataDictionary, nodeNamesDTO, isRoot);
        retrieved = CommonCodegenUtils.getMethodDeclaration(toPopulate.nodeTemplate, EVALUATE_PREDICATE).orElseThrow(() -> new RuntimeException("No EVALUATE_PREDICATE in generated class"));
        commonVerifyEvaluatePredicateMethodDeclaration(retrieved, nodeNamesDTO.nodeClassName, isRoot);
    }
    ...
    public void populateEvaluateNode() {
        ...
        // populated node
        isRoot = true;
        nodeNamesDTO = new KiePMMLNodeFactory.NodeNamesDTO(nodeRoot);
        toPopulate = new KiePMMLNodeFactory.JavaParserDTO(nodeNamesDTO, packageName);
        KiePMMLNodeFactory.populateEvaluateNode(toPopulate, nodeNamesDTO, isRoot);
        commonVerifyEvaluateNode(toPopulate, nodeNamesDTO, isRoot);
    }
    ...
    public void populateEvaluateNodeWithPredicateFunction() {
        ...
        KiePMMLNodeFactory.populateEvaluateNodeWithPredicateFunction(toPopulate, nodeClassName);
        commonVerifyEvaluateNodeWithPredicateFunction(variableDeclarator, nodeClassName);
    }
    ...
    public void getEvaluateNestedPredicateMethodDeclaration() {
        ...
        boolean isRoot = false;
        // nested node
        MethodDeclaration retrieved = KiePMMLNodeFactory.getEvaluateNestedPredicateMethodDeclaration(body, nodeName);
        commonVerifyEvaluatePredicateMethodDeclaration(retrieved, nodeName, isRoot);
    }
...
//    private void commonVerifyEvaluatePredicateMethodDeclaration(final MethodDeclaration retrieved, final String nodeName, final boolean isRoot) {
//        assertNotNull(retrieved);
...
//        expected = isRoot ? EVALUATE_PREDICATE : EVALUATE_PREDICATE + nodeName;
//        assertEquals(expected, retrieved.getName().asString());
//    }

    private void commonVerifyEvaluateNode(final KiePMMLNodeFactory.JavaParserDTO toPopulate, final KiePMMLNodeFactory.NodeNamesDTO nodeNamesDTO, final boolean isRoot) {
        BlockStmt blockStmt = isRoot ? toPopulate.evaluateRootNodeBody :  toPopulate.nodeTemplate.getMethodsByName(EVALUATE_NODE + nodeNamesDTO.nodeClassName ).get(0).getBody().orElseThrow(() -> new RuntimeException("No body in nested node evaluate node"));
        VariableDeclarator variableDeclarator = CommonCodegenUtils.getVariableDeclarator(blockStmt, PREDICATE_FUNCTION).orElseThrow(() -> new RuntimeException("No PREDICATE_FUNCTION variable declarator in generated methodCallExpr"));
        commonVerifyEvaluateNodeWithPredicateFunction(variableDeclarator, toPopulate.nodeClassName);
        variableDeclarator = CommonCodegenUtils.getVariableDeclarator(blockStmt, SCORE).orElseThrow(() -> new RuntimeException("No SCORE variable declarator in generated methodCallExpr"));
        commonVerifyEvaluateNodeWithScore(variableDeclarator, nodeNamesDTO.node.getScore());
        variableDeclarator = CommonCodegenUtils.getVariableDeclarator(blockStmt, NODE_FUNCTIONS).orElseThrow(() -> new RuntimeException("No NODE_FUNCTIONS variable declarator in generated methodCallExpr"));
        // TODO
//        Map<Node, String> nestedNodesClassMap = getNestedNodesClassMap(source);
//        commonVerifyEvaluateNodeWithNodeFunctions(variableDeclarator, KiePMMLNodeFactory.getNestedNodesFullClassNames(toPopulate, nestedNodesClassMap));
    }

    private void commonVerifyEvaluateNodeWithNodeFunctions(final VariableDeclarator variableDeclarator, final List<String> nestedNodesFullClasses) {
        ...
        Expression scope = methodCallExpr.getScope().orElseThrow(() -> new RuntimeException("No scope in generated methodCallExpr"));
        if (nestedNodesFullClasses.isEmpty()) {
            ...
            final NodeList<Expression> retrievedExpressions = methodCallExpr.getArguments();
            nestedNodesFullClasses.forEach(nestedNodesFullClass -> assertTrue(retrievedExpressions.stream()
                                                                                      .anyMatch(retrievedExpression -> retrievedExpression instanceof MethodReferenceExpr &&
                                                                                              nestedNodesFullClass.equals(((MethodReferenceExpr)retrievedExpression).getScope().toString()))));
        }
    }
    ...
    private void commonVerifyEvaluateNodeWithPredicateFunction(final VariableDeclarator variableDeclarator, final String nodeClassName) {
        assertTrue(variableDeclarator.getInitializer().isPresent());
        Expression expression = variableDeclarator.getInitializer().get();
        ...
        assertEquals(EVALUATE_PREDICATE, retrieved.getIdentifier());
    }
