...
----
    <pillarChangeMoveSelector>
      ...
      <subPillarSequenceComparatorClass>...</subPillarSequenceComparatorClass>
      ... <!-- Normal selector properties -->
      <pillarSelector>
        <entitySelector>
          <entityClass>...Lecture</entityClass>
...
There are several ways how sub pillars can be selected, driven by `subPillarType` property:

- `ANY` (default) will select all possible sub pillars.
- `SEQUENCE` will limit selection of sub pillars to <<sequentialSubPillars,Sequential sub pillars>>.
- `NONE` will never select sub pillars.
...
In this case, the entity being operated on need not be `Comparable`. The given `subPillarSequenceComparatorClass` will be used to establish the sequence instead. Also, the size of the sub pillars will be limited in length of up to 1000 entities.
