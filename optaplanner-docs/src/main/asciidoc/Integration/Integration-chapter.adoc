...
** In a prototype implementation, the human planner will use pinning occasionally to intervene.
But as the implementation matures, it should become obsolete.
But do keep the feature alive, as a reassurance for the humans.
Or in case that one day the business changes dramatically before the score constraints are adjusted accordingly.

Therefore, it's often a good idea to actively involve the human planner in your project.
