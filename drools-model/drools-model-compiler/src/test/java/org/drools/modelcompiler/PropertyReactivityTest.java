...
public class PropertyReactivityTest extends BaseModelTest {
    ...
    public void externalFunctionWithBindVariableFromAnotherPatternOfSameType_shouldTriggerClassReactive() {
        ...
        List<Message> warningList = kieBuilder.getResults().getMessages(Level.WARNING);
        assertThat(warningList).as("$id comes from a different pattern, so it triggers class reactivity. Warning on build time")
                               .hasSize(1);
        assertThat(warningList.get(0).getText()).contains("$id is not relevant to this pattern, so causes class reactivity.");
