package org.kie.pmml.models.mining.api.model.enums;
...
public enum MULTIPLE_MODEL_METHOD {
    ...
    WEIGHTED_AVERAGE("weightedAverage", inputData -> {
        ...
        valueWeightStream(inputData.values().stream(), "WEIGHTED_AVERAGE")
                .forEach(throwingConsumerWrapper(elem -> {
    ...
    WEIGHTED_SUM("x-weightedSum", inputData -> {
        return valueWeightStream(inputData.values().stream(), "WEIGHTED_SUM")
                .mapToDouble(throwingToDoubleFunctionWrapper(KiePMMLValueWeight::weightedValue)).sum();
    }),
     ...
     */
    private final ThrowingFunction<LinkedHashMap<String, KiePMMLNameValue>, Object, KieEnumException> function;

    MULTIPLE_MODEL_METHOD(String v, ThrowingFunction<LinkedHashMap<String, KiePMMLNameValue>, Object, KieEnumException> function) {
        name = v;
     ...
     */
    private static Stream<KiePMMLValueWeight> valueWeightStream(Stream<KiePMMLNameValue> toUnwrap, String enumName) throws KieEnumException {
        return toUnwrap.map(KiePMMLNameValue::getValue)
                .map(throwingFunctionWrapper(elem -> {
     ...
     * @throws KieEnumException
     */
    private static DoubleStream doubleStream(Stream<KiePMMLNameValue> toUnwrap, String enumName) throws KieEnumException {
     ...
     * @throws KieEnumException
     */
    public Object apply(LinkedHashMap<String, KiePMMLNameValue> inputData) throws KieEnumException {
