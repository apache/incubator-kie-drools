...
public class CellGridTest extends TestCase {
    ...
    public void testGiveBirth() {
    	grid.getCellAt(0, 0).setCellState(CellState.LIVE);
    	grid.getCellAt(0, 1).setCellState(CellState.LIVE);
    	grid.getCellAt(0, 2).setCellState(CellState.LIVE);
    	grid.nextGeneration();
    	assertEquals("Cell should have come to life", CellState.LIVE, grid.getCellAt(1, 1).getCellState());
    }

    public void testKillingTheLonely() {
    	grid.getCellAt(0, 0).setCellState(CellState.LIVE);
    	grid.nextGeneration();
    	assertEquals("Lonely cell should have been killed", CellState.DEAD, grid.getCellAt(0, 0).getCellState());
    }
    ...
    public void testKillingTheOvercrowded() {
    	...
    	grid.nextGeneration();
    	assertEquals("Overcrowded cell should have been killed", CellState.DEAD, grid.getCellAt(1, 1).getCellState());
    }
}
