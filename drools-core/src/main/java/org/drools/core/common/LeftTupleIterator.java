...
/*
        if (((InternalRuleBase)wm.getRuleBase()).getConfiguration().isPhreakEnabled()) {
            return new PhreakLeftTupleIterator(wm, node);
        }
*/
    ...
    public static class PhreakLeftTupleIterator extends LeftTupleIterator {

        private java.util.Iterator<LeftTuple> leftTuplesIterator;

        private PhreakLeftTupleIterator(InternalWorkingMemory wm,
                                        LeftTupleSink node) {
            super(wm, node);
        }
        ...
        public void setFirstLeftTupleForNode() {
            LeftTupleSource source = node.getLeftTupleSource();
            leftTuplesIterator = collectLeftTuples(source, node, wm).iterator();
            setNextLeftTuple();
        }

        @Override
        public void setNextLeftTuple() {
            currentLeftTuple = leftTuplesIterator.hasNext() ? leftTuplesIterator.next() : null;
        }

        @Override
        public LeftTuple getNextLeftTuple(LeftTupleSource source, LeftTupleSink node, LeftTuple currentLeftTuple, InternalWorkingMemory wm) {
            return super.getNextLeftTuple(source, node, currentLeftTuple, wm);
        }

        public List<LeftTuple> collectLeftTuples(LeftTupleSource node, LeftTupleSink peerNode, InternalWorkingMemory wm) {
        ...
        private void followPeerFromLeftInput(List<LeftTuple> leftTuples, LeftTuple lt, LeftTupleSink peerNode, List<LeftTupleSink> sinks, InternalWorkingMemory wm) {
            for (; lt != null; lt = lt.getLeftParentNext()) {
                followPeer(leftTuples, lt, sinks, sinks.size() -1, wm);
            }
        }

        private void followPeerFromRightInput(List<LeftTuple> leftTuples, LeftTuple lt, LeftTupleSink peerNode, List<LeftTupleSink> sinks, InternalWorkingMemory wm) {
            for (; lt != null; lt = lt.getRightParentNext()) {
                followPeer(leftTuples, lt, sinks,  sinks.size()-1, wm);
            }
        }

        private void followPeer(List<LeftTuple> leftTuples, LeftTuple lt, List<LeftTupleSink> sinks, int i, InternalWorkingMemory wm) {
            ...
            if ( i == 0 ) {
                ...
                LeftTuple peer = lt;
                while (peer != null && peer.getSink() != sink) {
                    peer = peer.getPeer();
                }
                if (peer != null) {
                    followPeer(leftTuples, lt, sinks, i-1, wm);
                }
            }
        }
    }
