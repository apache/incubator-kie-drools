...
public class DroolsObjectInputStream extends ObjectInputStream
    ...
    public void readExtractor( Consumer<InternalReadAccessor> binder ) throws ClassNotFoundException, IOException {
        ...
        if (accessor instanceof AccessorKey ) {
            ...
            if (reader == null) {
                ...
                List<Consumer<InternalReadAccessor>> list = extractorBinders.get((AccessorKey) accessor);
                if (list == null) {
                    list = new ArrayList<Consumer<InternalReadAccessor>>();
                    extractorBinders.put((AccessorKey) accessor, list);
                }
                list.add( binder );
            } else {
    ...
    public void bindAllExtractors(InternalKnowledgeBase kbase) {
        for (Map.Entry<AccessorKey, List<Consumer<InternalReadAccessor>>> entry : extractorBinders.entrySet()) {
