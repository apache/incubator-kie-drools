...
[NOTE]
====
Each optimization algorithm is represented by a solver `Phase`. There is never more than one `Phase` solving at the same time.
====
...
[NOTE]
====
If the `Solver` or a `Phase` wants to terminate while a `CustomPhaseCommand` is still running, it will wait to terminate until the `CustomPhaseCommand` is complete. This may take a significant amount of time.
