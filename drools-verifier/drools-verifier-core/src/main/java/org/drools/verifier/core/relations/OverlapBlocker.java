...
public class OverlapBlocker
        extends BlockerRelation<OverlapBlocker> {

    public static final OverlapBlocker EMPTY = new OverlapBlocker();

    public OverlapBlocker() {
        super();
    }

    public OverlapBlocker(final InspectorList list,
                          final HasUUID item) {
        super(list, item);
    }
    ...
    public boolean doesRelationStillExist() {
        if (origin != null
                && stillContainsConflictingItem(getOrigin().getItem())) {
    ...
    protected boolean stillContainsConflictingItem(final HasUUID item) {
