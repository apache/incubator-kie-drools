...
Because of JBoss Modules' `ClassLoader` magic, provide the `ClassLoader` of your classes xref:planner-configuration/planner-configuration.adoc#solverConfigurationByXML[during the SolverFactory creation],
so it can find the classpath resources (the solver config, domain classes, etc.) in your jars.

It's also recommended xref:optimization-algorithms/optimization-algorithms.adoc#customThreadFactory[to plug in WildFly's thread factory],
especially with xref:optimization-algorithms/optimization-algorithms.adoc#multithreadedSolving[multithreaded solving].


[[loggingOnWildFlyAndJBossEAP]]
==== Logging on WildFly and JBoss EAP

To get decent xref:planner-configuration/planner-configuration.adoc#logging[logging of the solver(s)], create a file `src/main/resources/jboss-log4j.xml`
...
Because of the OSGi's `ClassLoader` magic, provide the `ClassLoader` of your classes xref:planner-configuration/planner-configuration.adoc#solverConfigurationByXML[during the SolverFactory creation],
so it can find the classpath resources (the solver config, domain classes, etc.) in your jars.
...
Android is not a complete JVM (because some JDK libraries are missing), but OptaPlanner works on Android with xref:score-calculation/score-calculation.adoc#easyJavaScoreCalculation[easy Java] or xref:score-calculation/score-calculation.adoc#incrementalJavaScoreCalculation[incremental Java] score calculation.
