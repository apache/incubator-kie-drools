...
* Sequential ``ChangeMove``s: One variable is assigned at a time. Scales much better, especially for three or more variables.
...
This First Fit configuration for a single entity class with two variables, using sequential ``ChangeMove``s, will select 240 moves per entity:
