...
It <<sizingHardwareAndSoftware,saturates 5 CPU cores>>:
the 4 move threads fill up 4 CPU cores and the solver thread itself takes another CPU core.
...
On machines or containers with little or no CPU's, this can fall back to the single threaded code.
...
A run of the same solver configuration on 2 machines with a different number of CPU's,
is still reproducible, unless the `moveThreadCount` is set to `AUTO` or a function of `availableProcessorCount`.
====

The `moveThreadBufferSize` can be used to power tweak the number of moves that are selected but won't be foraged.
Setting it too low will reduce performance, setting it too high might too.
Unless you're deeply familiar with the inner workings of multithreaded solving, don't configure this parameter.

To run in a environment that doesn't like arbitrary thread creation,
